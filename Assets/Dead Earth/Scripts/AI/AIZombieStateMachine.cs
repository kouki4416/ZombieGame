...
public class AIZombieStateMachine : AIStateMachine
{
	...
	private float _nextRadgollSoundTime = 0.0f;//This tells when we can make new sound
	...
	public override void TakeDamage( Vector3 position, Vector3 force, int damage, Rigidbody bodyPart, CharacterManager characterManager, int hitDirection=0 )
	{
		...
		if (_boneControlType==AIBoneControlType.Ragdoll)
		{
			if (bodyPart!=null)
			{
				//play ragdoll damage sound
				if(Time.time > _nextRadgollSoundTime && _ragdollCollection!=null && _health>0)
				{
					AudioClip clip = _ragdollCollection[1];
					if(clip)
					{
						_nextRadgollSoundTime = Time.time + clip.length;
		...
		else // Ragdoll, zombie just drops
		{
			if (_currentState)
			{
			...
			//play dramatic ragdoll sound
			if (Time.time > _nextRadgollSoundTime && _ragdollCollection != null && prevHealth > 0)
			{
